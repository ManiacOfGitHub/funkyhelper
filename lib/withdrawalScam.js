...
module.exports = (client, logChannel, config) => {
    ...
    async function onMessage(message) {
        ...
        if((dangerUserId == message.member.id) && scamDetectedAndBanned) {
            try {
                await message.delete();
                await logChannel.send("A message in <#"+message.channel.id+"> from " + message.member.mention + "(" + message.member.id + ") was caught after the user was temporarily banned and has now been deleted. (Note: This was done via `dangerUserId`, which means that this message was processed/sent after the user had already been banned.)");
            } catch(err) {}
            return;
        }
        ...
        for(var i in attachments) {
            await logChannel.send("Downloading attachment " + (parseInt(i) + 1) + " of " + (message.attachments.size));
            ...
            await logChannel.send("Testing attachment " + (parseInt(i) + 1) + " of " + (message.attachments.size));
        ...
        await logChannel.send("Ban succeeeded. Waiting 15 seconds to catch all messages...");
        await new Promise(resolve=>{setTimeout(resolve,15000)});
        await logChannel.send("Attempting to delete any stray messages...");
        for(var i of tempSusMessages) {
            try {
                await i.delete();
                await logChannel.send("A message was caught after the user was temporarily banned and has now been deleted. (Dev Note: This was done via `tempSusMessages`.)");
            } catch(err) {}
        }
        await logChannel.send("Attempt complete. Attempting to unban user...");
        try {
